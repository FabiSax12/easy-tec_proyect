...
import { Task, TaskWithCourseName } from "@/domain/entities/Task"
import { createTaskService } from "@/application/services/TaskService"
import { taskApiRepository } from "@/infraestructure/api/TaskApiRepository"
import { TaskModal, TaskCardInfo } from "@/presentation/task/components"

const taskService = createTaskService(taskApiRepository)
...
export function TaskCard({ task, handleTaskUpdate, handleTaskDelete, handleTaskAdd }: Props) {
  ...
  const onDeleteTask = async () => {
    toast.promise(() => taskService.deleteTask(task.id), {
